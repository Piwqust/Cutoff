...
def main() -> int:
    ...
    ap.add_argument("--multi-opener-8max", action="store_true",
                    help="Emit per-opener vsopen charts (1 file per "
                         "(defender, opener) pair). Output filenames: "
                         "mtt_8max_<bb>_<defender>_vsopen_<opener>.csv. "
                         "These do NOT feed the Swift importer's current "
                         "Filename.parse() (which requires 5 underscore-"
                         "separated parts) — they're reference data for "
                         "future multi-opener app integration.")
    args = ap.parse_args()
    ...
    if args.canonical_8max:
        ...
        # Compose a compact, app-consumable tree-param summary. The Swift
        # CribSheet parser harvests `# TreeParams: ...` and the Emitter
        # surfaces it as the JSON's source.publisher.treeParams field.
        sizes = meta["preflop_sizes"]
        if isinstance(sizes, dict):
            tree_params_summary = (
                f"HRC v3 Advanced ChipEV @ {depth_bb}bb: "
                f"open {sizes.get('SIZES_OPEN_OTHERS', '?')}; "
                f"BU {sizes.get('SIZES_OPEN_BU', '?')}; "
                f"SB {sizes.get('SIZES_OPEN_SB', '?')}; "
                f"3-bet IP {sizes.get('SIZES_3BET_IP', '?')}; "
                f"3-bet BB-vs-others {sizes.get('SIZES_3BET_BB_VS_OTHER', '?')}; "
                f"4-bet IP {sizes.get('SIZES_4BET_IP', '?')}"
            )
        else:
            tree_params_summary = (
                f"HRC v3 Simple ChipEV @ {depth_bb}bb: "
                f"PREFLOP_SIZES={sizes}"
            )
        ...
        prov_base = [
            ...
            f"TreeParams: {tree_params_summary}",
        ]
    ...
    # ---------------- Multi-opener 8-max mode ---------------- #
    if args.multi_opener_8max:
        if meta["n_seats"] != 8:
            print(f"WARNING: --multi-opener-8max expects 8-max solve; got "
                  f"{meta['n_seats']}-handed.", file=sys.stderr)
        depth_bb = int(round(meta["eff_bb"]))
        prov_base = [
            f"Generated by hrc_import.py --multi-opener-8max from {args.input.name}",
            f"HRC tree: {meta['n_seats']}-handed, "
            f"effective {meta['eff_bb']}bb",
            f"Equity model: {meta['eqmodel']}, ante: {meta['ante_type']}",
        ]

        written = 0
        # For each (defender, opener) pair where opener < defender, walk
        # the tree to: [F]*opener + [R] + [F]*(defender - opener - 1).
        # That sequence describes "all earlier players folded; opener raised;
        # all players between opener and defender folded; defender's turn".
        for defender_seat in range(1, 8):  # UTG1..BB
            defender_name = POSITION_BY_SEAT_8MAX[defender_seat]
            for opener_seat in range(defender_seat):  # 0..(defender-1)
                opener_name = POSITION_BY_SEAT_8MAX[opener_seat]
                seq = (["F"] * opener_seat
                       + ["R"]
                       + ["F"] * (defender_seat - opener_seat - 1))
                nid = find_node_by_seq(nodes, seq)
                if nid is None:
                    continue
                node = nodes[nid]
                if node.get("player") != defender_seat:
                    continue
                labels, rows, vpip = node_to_rows(node, stack_chips)
                if not rows:
                    continue
                fname = (f"mtt_8max_{depth_bb}bb_{defender_name}"
                         f"_vsopen_{opener_name}.csv")
                out_path = args.output / fname
                prov = prov_base + [
                    f"App slot (deferred integration): {defender_name} vsopen "
                    f"vs {opener_name} open @ {depth_bb}bb",
                    f"HRC node: id={nid}, seat={defender_seat}, "
                    f"sequence={seq_code(node.get('sequence', []))}",
                    f"Actions: {labels}",
                    f"VPIP estimate: {vpip*100:.2f}%",
                ]
                write_csv(out_path, rows, prov)
                written += 1
                if args.print_summary:
                    print(f"  {fname:<48} VPIP {vpip*100:5.2f}%  rows {len(rows):3d}")
        print(f"Wrote {written} multi-opener CSVs to {args.output}")
        return 0
    # ------------- End multi-opener 8-max mode --------------- #
