...
struct DeveloperStrategyDocView: View {
    ...
    ## 7. Поиск и Исследования (Search & Research)
    Если вы передаете задачи ИИ-ассистентам или разработчикам, придерживайтесь этих инструкций:
    
    ### Поисковые запросы (Search Queries)
    Для поиска релевантного кода используйте следующие ключевые слова:
    - **Новые статьи/главы**: Ищите `StrategyStore`, `WeeklyGuide` или `StrategyChapter`. Здесь хранятся все данные.
    - **UI Компоненты**: Ищите `RichCardText`, `CardView` или `ComboView`.
    - **Локализация**: Ищите `LocalizationManager` или `AppTranslations.swift`.
    
    ### Входные данные для разработчиков (Input Data)
    При передаче новых статей разработчикам (или ИИ-ассистентам), предоставляйте текст в сыром виде со вставленными нотациями (п.1 - п.5). В коде статьи формируются массивом `StrategySection`. 
    Для быстрой интеграции предоставляйте данные в таком формате:
    ```swift
    StrategyChapter(
        id: "chapter-id",
        title: "Название",
        shortDescription: "Краткое описание",
        tag: .theory, // .theory, .math, .exploits
        content: [
            StrategySection(
                title: "Подзаголовок",
                body: "Ваш текст с нотациями (например, Ah-Kd-2c ).",
                image: nil // Или "image_name" если есть картинка в Assets
            )
        ]
    )
    ```
